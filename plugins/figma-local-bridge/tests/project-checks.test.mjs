...
test('OpenCode launcher сохраняет cwd, аргументы и код завершения', async t => {
  ...
  const args = windows ? ['/d', '/s', '/c', `""${join(root, 'START_OPENCODE.cmd')}" "probe with spaces" tail"`] : [join(root, 'scripts/start-opencode.sh'), 'probe with spaces', 'tail'];
  // The /c command already uses cmd.exe quoting. Node's default Windows
  // argument escaping would add backslashes and prevent the launcher running.
  await assert.rejects(exec(command, args, { cwd: temporary, env, windowsVerbatimArguments: windows }), error => {
    assert.equal(error.code, 37, JSON.stringify({ code: error.code, stdout: error.stdout, stderr: error.stderr }));
    assert.deepEqual(error.stdout.trim().split(/\r?\n/), [root, 'probe with spaces', 'tail']);
    return true;
  });
});
